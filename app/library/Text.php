...
class Text
{
    ...
    public static function word_limiter($str, $limit = 100, $end_char = '&#8230;')
    {
        ...
        $end_char = (String::strlen($str) == String::strlen($matches[0])) ? '' : $end_char;
     ...
     * @return string обработанная строка
     */
    public static function character_limiter($str, $limit = 500, $end_char = '&#8230;', $max_word_lench = 500,$allowTags = null)
    {
        
        $str = strip_tags($str,$allowTags);
        
        if (String::strlen($str) < $limit) {
            return $str;
        }
        ...
        $out = "";
        foreach (explode(' ', String::trim($str)) as $val) {
            if (String::strlen($val) > $max_word_lench) {
                $val = String::substr($val, 0, $max_word_lench) . $end_char;
            }
            $out .= $val . ' ';

            if (String::strlen($out) >= $limit) {
                $out = String::trim($out);

                return (String::strlen($out) == String::strlen($str)) ? $out : $out . $end_char;
            }
        }

        return String::substr($str, 0, $limit) . $end_char;
    }
     ...
     *
     * @tutorial Text::text_to_url( 'Compellingly strategize B2B portals after empowered.' ); => compellingly-strategize-b2b-portals-after-empowered
     * @tutorial Text::text_to_url( 'Гендер теоретически возможен!' ); => gender-teoreticheski-vozmozhen
     * @tutorial Text::text_to_url( 'Разного рода символы !%#$&^&*^%*&)()(*_)*--  <> -' ); => raznogo-roda-simvoly
     *
    ...
    public static function text_wrap($text, $max_length = 30)
    {
        ...
        $textLength = String::strlen($text);

        for ($i = 0; $i <= $textLength; $i++) {
            $array[] = String::substr($text, $i, 1);
        }
    ...
    public static function to_canonical($text)
    {
        ...
        $return = $o = '';
        $_l = String::strlen($text);
        for ($i = 0; $i < $_l; $i++) {
            $c = String::substr($text, $i, 1);
            if ($c != $o) {
                $return .= $c;
                $o = $c;
            }
     ...
     *
     *
     * @tutorial Text::sexerate( 1, array('умник','умница','умницо') );
     * @tutorial Text::sexerate( 'm', array('делал','делала','делало') )
     *
     ...
     *
     * @tutorial  Text::json_encode( array(1=>'Один',2=>'Два') ); => {"1":"Один","2":"Два"}
     *
